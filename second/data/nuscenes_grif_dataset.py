...
@register_dataset
class NuScenesGRIFDataset(Dataset):
    # NumPointFeatures = 4  # xyz, timestamp. set 4 to use kitti pretrain
    # TODO: x y z rcs vx_comp vy_comp + timestamp (TODO)
    NumPointFeatures = 6
    NameMapping = {
    ...
    def __getitem__(self, idx):
        ...
        # TODO: get radar data
        input_dict = self.get_sensor_data_radar(idx)
    ...
    # TODO: lidar to radar here, add timestamp channel to radar
    def get_sensor_data_radar(self, query):
        idx = query
        ...
        points = np.fromfile(str(radar_path), dtype=np.float32, count=-1).reshape([-1, 6])
    ...
    def get_sensor_data(self, query):
        ...
        idx = np.random.choice(len(points),1200,replace=False)
        points = points[idx,:]
        points[:, 3] /= 255
        points[:, 4] = 0
        ...
        for det in detections:
            ...
            for i, box in enumerate(boxes):
                ...
                nusc_anno = {
                    ...
                    "attribute_name": NuScenesGRIFDataset.DefaultAttribute[name],
                }
...
@register_dataset
class NuScenesGRIFDatasetD8(NuScenesGRIFDataset):
...
@register_dataset
class NuScenesGRIFDatasetD8Velo(NuScenesGRIFDatasetD8):
...
@register_dataset
class NuScenesGRIFDatasetVelo(NuScenesGRIFDataset):
    def __init__(self, *args, **kw):
        super().__init__(*args, **kw)
        self._with_velocity = True
...
@register_dataset
class NuScenesGRIFDatasetD7(NuScenesGRIFDataset):
    def __init__(self, *args, **kw):
        super().__init__(*args, **kw)
        if len(self._nusc_infos) > 28000:
            self._nusc_infos = list(
...
@register_dataset
class NuScenesGRIFDatasetD6(NuScenesGRIFDataset):
    def __init__(self, *args, **kw):
        super().__init__(*args, **kw)
        if len(self._nusc_infos) > 28000:
            self._nusc_infos = list(
...
@register_dataset
class NuScenesGRIFDatasetD5(NuScenesGRIFDataset):
    def __init__(self, *args, **kw):
        super().__init__(*args, **kw)
        if len(self._nusc_infos) > 28000:
            self._nusc_infos = list(
...
@register_dataset
class NuScenesGRIFDatasetD4(NuScenesGRIFDataset):
    def __init__(self, *args, **kw):
        super().__init__(*args, **kw)
        if len(self._nusc_infos) > 28000:
            self._nusc_infos = list(
...
@register_dataset
class NuScenesGRIFDatasetD3(NuScenesGRIFDataset):
    def __init__(self, *args, **kw):
        super().__init__(*args, **kw)
        if len(self._nusc_infos) > 28000:
            self._nusc_infos = list(
...
@register_dataset
class NuScenesGRIFDatasetD2(NuScenesGRIFDataset):
    def __init__(self, *args, **kw):
        super().__init__(*args, **kw)
        if len(self._nusc_infos) > 28000:
            self._nusc_infos = list(
...
@register_dataset
class NuScenesGRIFDatasetD2Velo(NuScenesGRIFDatasetD2):
    def __init__(self, *args, **kw):
        super().__init__(*args, **kw)
        self._with_velocity = True
...
def _fill_trainval_infos(nusc,
    ...
    for sample in prog_bar(nusc.sample):
        ...
        if sample["scene_token"] in train_scenes:
            radar_path = '/mnt/sdd/jhyoo/dataset/NUSCENES/ns_radar_train/radar/%06d.bin' %idx
            # depth_path = '/mnt/sda/jskim/OpenPCDet/training/ns_radar_train/radar/%06d.bin' %idx
        else:
            radar_path = '/mnt/sdd/jhyoo/dataset/NUSCENES/ns_radar_val/radar/%06d.bin' %idx
            # depth_path = '/mnt/sda/jskim/OpenPCDet/training/ns_radar_train/radar/%06d.bin' %idx
        idx += 1
        ...
        if not test:
            ...
            for i in range(len(names)):
                if names[i] in NuScenesGRIFDataset.NameMapping:
                    names[i] = NuScenesGRIFDataset.NameMapping[names[i]]
...
def create_nuscenes_infos(root_path, radar_version, version="v1.0-trainval", max_sweeps=10):
    ...
    output_path = Path('/home/spalab/jskim_2/second.pytorch/second/dataset')
