...
from second.data.ds_sample import ds_sampling
...
class NuScenesRadarDataset(Dataset):
    ...
    def ground_truth_bbox(self):
        ...
        for info in self._nusc_infos:
            ...
            annos=[]
            boxes = _second_gt_to_nusc_box(info)
    ...
    def evaluation_nusc(self, detections, output_dir):
        ...
        gt_annos = self.ground_truth_annotations
        gt_bboxes = self.ground_truth_bbox
        if gt_annos is None:
            return None
        nusc_annos = {}
        mapped_class_names = self._class_names
        ...
        gt_path = Path(output_dir) / "gt_nusc.json"
...
def _second_gt_to_nusc_box(info):
...
def make_multisweep_radar_data(nusc, nusc_can, sample, radar_version, max_sweeps, future_tokens):
    ...
    for radar_channel in radar_channels:
        ...
        for _ in range(max_sweeps):
            ...
            # import pdb; pdb.set_trace()
            if radar_version in ['version3','versionx3','versionx2']:
            ...
            elif radar_version in ['versionx2']:
                versionx2_pc[:,:3] = versionx2_pc[:,:3] @ R
                versionx2_pc[:,[8,9]] = versionx2_pc[:,[8,9]] @ R[:2,:2]
                versionx2_pc[:,:3] += T
                all_pc = np.vstack((all_pc,versionx2_pc))
            else:
    ...
    if radar_version in ['version1','version2','version3','versionx3','versionx2']:
        use_idx = [0,1,2,5,8,9]
    else:
        raise NotImplementedError
...
def _fill_trainval_infos(nusc,nusc_can,
    ...
    for sample in prog_bar(total_samples):
        ...
        if modality == 'radar':
            ...
            ref_radar_path_split[-2] = 'SECOND_data/RADAR_' + radar_version + '_'+ str(max_sweeps) + 'Sweeps'
            ...
            if not os.path.exists(dir_data):
                os.mkdir(dir_data)
            radar_npy = make_multisweep_radar_data(nusc,nusc_can,sample,radar_version,max_sweeps,future_tokens)
        ...
        if not test:
            ...
            points = radar_npy
            indices = box_np_ops.points_count_rbbox(points,gt_boxes)
            ...
            try:
                ...
                info["gt_velocity"] = velocity.reshape(-1, 2)[mask]
                info["num_lidar_pts"] = np.array([a["num_lidar_pts"] for a in annotations])[mask]
                info["num_radar_pts"] = radar_points
            except:
...
def create_nuscenes_infos(root_path, modality ,radar_version, version="v1.0-trainval" ,max_sweeps=10, ds_sample=False):
    ...
    output_path = Path(f'/home/spalab/jskim_2/second.pytorch/second/dataset/{max_sweeps}sweeps')
    ...
    train_nusc_infos, val_nusc_infos = _fill_trainval_infos(
        nusc, nusc_can, train_scenes, val_scenes, modality, radar_version, future_tokens,test=test, max_sweeps=max_sweeps)
    
    if ds_sample:
        ds_nusc_infos = ds_sampling(train_nusc_infos)
    metadata = {
        "version": version,
    }
    if test:
        ...
        if ds_sample:
            ...
            with open(output_path / f"infos_train_multisweep_{radar_version}_sweeps{max_sweeps}_ds.pkl", 'wb') as f:
